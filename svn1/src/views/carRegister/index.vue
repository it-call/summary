        ...
        <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="14">
          <div class="grid-content bg-purple">
            ...
            >
              ...
              <el-table-column label="车牌" width="120" prop="date">
                  <template slot-scope="scope" >
                         <el-input v-if="scope.row.status" v-model="scope.row.date"></el-input>
                       <span v-else>{{scope.row.date}}</span>
                  </template>
                <!-- <el-input v-if="bianji">test</el-input> -->
              </el-table-column>
              ...
              <el-table-column label="企业">
                <!-- <template slot-scope="scope" prop="date">{{ scope.row.address }}</template> -->
                    <template slot-scope="scope" prop='address'>
                         <!-- <el-input v-if="scope.row.status" v-model="scope.row.address" ></el-input> -->
  ...
  methods: {
    handleEdit(index, row) {
     ...
     baocun(index, row) {
      ...
      console.log('之前的数据',this.zhuceTable);
      console.log(row.status,'当前的状态');
    this.zhuceTable.forEach(item=>{
      if(item.index==index){
        item.licensePlate=row.date;
      }
    })
      if(row.status==1){
        console.log('状态为一的时候');
    ...
    zhuce() {
      ...
      console.log("发送的参数",this.zhuceTable);
      if(this.zhuceTable.length==[]){
        this.$message({
          message:'请先勾选',
          type:'warning'
        })
